...
console.log('#'+this.parentEditForm+" .modal-footer");
        ...
        for (i =0; i< testInstance.dataTableReferences.length ; i++) {
...
//--------------------------------------------------------------------------------------------------------
       ...
       $('<div>',{class:'col-sm-12'}).append($('<a>',{class:'btn btn-primary',id:'create-button',href:'#',role:'button',text:'agregar'})[0]);
        ...
        this.insertForm.appendChild($('<div>',{class:'col-sm-12 col-sm-offset-8'}).append(b)[0]);
...
myTable=$('<table>',{id:parentTable+this.modelName+"DataTable",class:"cell-border",cellspacing:"0",width:"100%"}).append($('<thead>').append($('<tr>')));
tr=myTable.children().children();
  ...
  crud.prototype.postFormAjaxRequest=function(){
    ...
    for(i=0;i<this.insertFormModel.length;i++){
      ...
      if(this.insertFormModel[i][keyname].id=="archivo"){
          jsonString=jsonString +'"'+this.insertFormModel[i][keyname].id+'":"'+$('input[type=file]')[0].files[0].name+'",';
...
crud.prototype.llenarSelect=function(targetId,dataArray,targetValueKey,targetTextKey){
...
crud.prototype.crearSelect=function(modelRoute,targetId,targetValueKey,targetTextKey){
   ...
   $.get({url:modelRoute,
    ...
    success:function(res){
      ...
      console.log(JSON.stringify(res));
      if(!res){console.log("nop"); return};
      ObjectPointer.llenarSelect(targetId,res,targetValueKey,targetTextKey);
