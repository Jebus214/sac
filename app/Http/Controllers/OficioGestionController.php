...
class OficioGestionController extends Controller
{
    //
...
public function changeStatusEnviar(Request $request)
...
public function updateTurnado(Request $request)

    {
         //$area_id = intval(Auth::user()->area_id);
       

        $oficio=Oficio::findOrFail($request->input('oficio_id'));



         $clavesDestinatarios= json_decode('[' .$request->input('destinatario_id'). ']', true);
         $oficio->destinatarios()->sync($clavesDestinatarios);   


         if($request->input('copia_id')!='null'){
             $claves= json_decode('[' .$request->input('copia_id'). ']', true);
             $oficio->cpersona()->sync($claves);
         }
